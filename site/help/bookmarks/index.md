...
In Phi the sidebar is your workspace, and bookmarks and pinned tabs are the two ways you keep the pages that matter. This chapter goes deeper than [Layouts & navigation](/help/layouts/) on how they behave — starting with the one thing that surprises people coming from Chrome: in Phi, a bookmark opens in place as a tab.
...
Bookmarks are scoped to the Space you are in — each [Space](/help/spaces/) keeps its own set, so a work Space's saved pages do not clutter a personal one.
...
Pinned tabs belong to your **Profile**, not to a single Space, so they appear across every Space that shares that Profile. Bookmarks, by contrast, are per Space — see [Spaces & Profiles](/help/spaces/) for how those layers fit together.
